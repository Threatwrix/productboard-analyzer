...
export class Reporter {
  constructor() {
    this.colors = {
      fresh: chalk.green,
      stale: chalk.yellow,
      outdated: chalk.hex('#FFA500'), // orange color
      ancient: chalk.red,
  ...
  printSummary(analysis) {
    ...
    table.push(
      ...
      [this.colors.outdated('Outdated (6-12 months)'), summary.outdatedItems, `${((summary.outdatedItems / total) * 100).toFixed(1)}%`],
    ...
    for (const [type, data] of Object.entries(analysis.byType)) {
      if (data.total > 0) {
        table.push([
          ...
          this.colors.outdated(data.outdated),
  ...
  async printHygieneScore(analysis) {
    ...
    if (score >= 80) {
      ...
      scoreColor = this.colors.outdated;
