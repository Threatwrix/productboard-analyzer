...
export class Reporter {
  ...
  generateReport(analysis) {
    ...
    this.printHygieneScore(analysis);
  }
  ...
  printSummary(analysis) {
    ...
    table.push(
      ...
      [this.colors.colors.orange('Outdated (6-12 months)'), summary.outdatedItems, `${((summary.outdatedItems / total) * 100).toFixed(1)}%`],
    ...
    for (const [type, data] of Object.entries(analysis.byType)) {
      if (data.total > 0) {
        table.push([
          ...
          this.colors.colors.orange(data.outdated),
  ...
  printHygieneScore(analysis) {
    ...
    if (score >= 80) {
      ...
      scoreColor = this.colors.colors.orange;
